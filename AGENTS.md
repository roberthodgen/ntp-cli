...
- Build release binaries for Windows, macOS, and Linux with `make ntpc`; this publishes self-contained single-file outputs and copies them to root as `ntpc_win-x64.exe`, `ntpc_osx-x64`, and `ntpc_linux-x64`.
- Remove release outputs with `make clean`.

## Repo-Specific Gotchas
- Root `ntpc_*` binaries are build artifacts listed in `.gitignore`; do not edit or commit generated binaries.
